...
class MappedList {
  ...
  create() {
    ...
    const data = symbols.reduce((acc, symbol) => {
      ...
      const spriteFilenames = module.chunks.map(chunk => spriteFilename.replace('[chunkname]', chunk.name));

      if (rule && module) {
        spriteFilenames.forEach(spriteFilename => acc.push(new MappedListItem(symbol, module, spriteFilename)));
      }
